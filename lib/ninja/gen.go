...
type Generator struct {
	...
	// Memoization caches for recursive collection functions (reset each Generate call)
	memoIncludes map[string][]string
	memoCflags   map[string][]string
	memoLdflags  map[string][]string
}
...
func (g *Generator) collectIncludePaths(moduleName string, visited map[string]bool) []string {
	if cached, ok := g.memoIncludes[moduleName]; ok {
		return cached
	}
	...
	g.memoIncludes[moduleName] = includes
	return includes
}
...
func (g *Generator) collectExportCflags(moduleName string, visited map[string]bool) []string {
	if cached, ok := g.memoCflags[moduleName]; ok {
		return cached
	}
...
func (g *Generator) collectExportLdflags(moduleName string, visited map[string]bool) []string {
	if cached, ok := g.memoLdflags[moduleName]; ok {
		return cached
	}
	...
	g.memoLdflags[moduleName] = flags
	return flags
}
...
func (g *Generator) Generate(w io.Writer) error {
	// Reset memoization caches for this generation pass
	g.memoIncludes = make(map[string][]string)
	g.memoCflags = make(map[string][]string)
	g.memoLdflags = make(map[string][]string)
